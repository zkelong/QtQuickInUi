...
Window58::Window58(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::Window58)
{
    ui->setupUi(this);
    ...
    QPixmap pix(":res/yafeilinux.png");
    // 添加图片
    label->setPixmap(pix);
    // 设置标签大小为图片的大小
...
// 鼠标按下事件
void Window58::mousePressEvent(QMouseEvent *event)
{
    /*****第一步：获取图片******/
...
// 拖动进入事件
void Window58::dragEnterEvent(QDragEnterEvent *event)
{
...
// 拖动事件
void Window58::dragMoveEvent(QDragMoveEvent *event)
{
     if (event->mimeData()->hasFormat("myimage/png")) {
             event->setDropAction(Qt::MoveAction);
             event->accept();
...
// 放下事件
void Window58::dropEvent(QDropEvent *event)
{
    if (event->mimeData()->hasFormat("myimage/png")) {
